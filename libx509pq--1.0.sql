...
-- All functions are pure (no shared state, no SPI) and therefore PARALLEL SAFE.
-- Functions whose optional integer/boolean argument uses NULL as a sentinel
-- for "use the default flags" cannot be declared STRICT; all others are.
...
CREATE OR REPLACE FUNCTION x509_authorityKeyId(bytea) RETURNS bytea
	AS '$libdir/libx509pq.so' LANGUAGE c IMMUTABLE STRICT PARALLEL SAFE;
